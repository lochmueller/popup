...
class tx_popup {
	...
	public function getPopupTarget($pageOrPageID) {
		if (Tx_Popup_Utility::checkInt($pageOrPageID)) {
			$pageOrPageID = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'pages', 'uid=' . $pageOrPageID, '', '', 1));
		} # if
	...
	function textParse($content, $conf) {
		...
		if ($conf['link'] OR is_array($conf['link.'])) {
			...
			$JSpopup['properties.'] = $this->array_merge_recursive_overrule($JSpopup['properties.'], $this->cObj->parameters, TRUE);
		}
		...
		$TAG = '<a href="' . $JSpopup['link'] . '" onClick="openPic(this.href,\'' . str_replace(' ', '', $JSpopup['windowname']) . '\',\'' . $props . '\'); return false;" class="linkpop"' . $JSpopup['ATagParams'] . '>';
		if ($linkContent) {
			$TAG .= $linkContent . '</a>';
		}
	...
	public function convertJs2Cfg($string) {
		return $this->convertArray2Cfg($this->convertJs2Array($string, array()));
