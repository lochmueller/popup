...
class ux_tslib_cObj extends tslib_cObj {
	...
	function typoLink($linktxt, $conf)	{
		...
		$sectionMark = $sectionMark ? (Tx_Popup_Utility::checkInt($sectionMark)?'#c':'#').$sectionMark : '';
		...
		if ($link_param) {
			...
			if(strstr($link_param,'@') && (!$pU['scheme'] || $pU['scheme']=='mailto'))	{		// If it's a mail address:
				...
				if (!Tx_Popup_Utility::checkInt($link_param) || count($GLOBALS['TSFE']->sys_page->getPage_noCheck($link_param)) == 0) {
				...
				if($pU['scheme'] || ($isLocalFile!=1 && $urlChar && (!$containsSlash || $urlChar<$fileChar)))	{	// url (external): If doubleSlash or if a '.' comes before a '/'.
					...
					if ($link_params_parts[1] && !$sectionMark)	{
						...
						$sectionMark = (Tx_Popup_Utility::checkInt($sectionMark)?'#c':'#').$sectionMark;
					}
					...
					if (!Tx_Popup_Utility::checkInt($link_param))	{
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/popup/xclasses/class.ux_tslib_cObj.php']) {
	include_once ($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/popup/xclasses/class.ux_tslib_cObj.php']);
} # if
?>