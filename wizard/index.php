...
class tx_popup_wiz extends t3lib_SCbase {
	...
	function moduleContent($content = '') {
		...
		// Configuration
		$pageID = Tx_Popup_Utility::checkInt($_GET['P']['uid'])?$_GET['P']['uid']:$_GET['P']['pid'];
		$advanced = (isset($_GET['advanced']) && intval($_GET['advanced']) === 1)?true:false;
		$current = (isset($_GET['P']['currentValue']) && trim($_GET['P']['currentValue']) != '')?$_GET['P']['currentValue']:$this->popup->convertArray2Cfg($this->popup->getDefaultConfiguration($pageID, $advanced), $advanced);
		...
		if($advanced) {
			...
			$this->content .= $this->doc->section($LANG->getLL('advanced_configuration').':',$content.'</table>',0,1);
		} # if
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/popup/wizard/index.php'])    {
    include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/popup/wizard/index.php']);
} # if
